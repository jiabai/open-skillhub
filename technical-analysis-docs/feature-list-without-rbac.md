...
> 基于 `ENABLE_RBAC=False`（默认配置）下的完整功能盘点。**任何已认证用户均可使用以下全部功能** —— 唯一门槛是有效的 JWT Token。
...
**关键认知**：RBAC 关闭后，角色概念（admin/member/viewer）完全消失。系统的安全模型退化为两层——**"你是谁"**（JWT 身份）+ **"这是不是你的东西"**（所有权检查）。所有需要认证的接口对已登录用户一视同仁。
...
分为**个人操作**和**管理操作**两部分。RBAC 关闭后，管理操作也对普通用户开放。

```mermaid
flowchart TD
    subgraph 个人操作["个人操作（操作自己）"]
        ME_GET[查看我的信息]
        ME_PUT[修改我的信息]
        BIND[更换绑定邮箱]
        DEL_REQ[请求注销]
    ...
    subgraph 管理操作["管理操作（RBAC关闭后人人可用）"]
        LIST[查看全部用户列表]
        IDENTITY[修改他人身份/角色]
    end
    ...
    style 管理操作 fill:#fff3e0,stroke:#ef6c00
```
...
| `/api/v1/users` | GET | 管理 | 分页查询系统中所有用户 ⚠️ |
| `/api/v1/users/{id}/identity` | PUT | 管理 | 修改任意用户的角色或状态 ⚠️ |

⚠️ 标记的接口在 RBAC 开启时仅 admin 可用，关闭后无角色限制。
...
> RBAC 开启时，下载仅 admin 可用；RBAC 关闭后，**所有已认证用户均可下载自己的 Skill 以及可见的公共 Skill**。
...
| 文件扩展名 | 33 种白名单 | 返回 400 INVALID_FILENAME |
...
RBAC 关闭的本质影响是：**所有 `require_permission()` 检查短路返回 True**。大部分接口本身就需要 member 或 viewer 角色（覆盖了绝大多数正常用户），所以实际感知变化有限。真正产生影响的只有那些原本限定给 admin 的操作。

### 有实质变化的 6 个功能点

| # | 功能 | RBAC 开启时 | RBAC 关闭后 | 风险等级 | 说明 |
|---|------|------------|------------|----------|------|
| 1 | **Skill 下载** | 仅 admin | 所有已认证用户 | 🟡 中高 | 最常被感知的变化，下载功能从管理员特权变为通用能力 |
| 2 | **用户列表** | 仅 admin | 所有已认证用户 | 🔴 高 | 可枚举系统中全部用户信息 |
| 3 | **修改他人身份** | 仅 admin | 所有已认证用户 | 🔴 极高 | 可将自己或其他用户提升为任意角色 |
| 4 | **查看审计日志** | 仅 admin | 所有已认证用户 | 🟡 中 | 可查看他人的操作记录 |
| 5 | **导出审计日志** | 仅 admin | 所有已认证用户 | 🟡 中 | 可批量导出最多 1000 条审计数据 |
| 6 | **清理/重置指标** | 仅 admin | 所有已认证用户 | 🟢 低 | 可删除统计数据，不影响业务数据 |

### 无变化的功能（本身就是 member+ 或 viewer+ 可用）

Skill 的列表、读取、创建、更新、删除、上传、执行——这些操作在 RBAC 开启时就对 member 角色开放，而默认角色就是 member，所以关闭 RBAC 后没有任何感知差异。
...
flowchart TD
    ...
    L1 --> L2{"第2层: 所有权校验<br/>_ensure_owner()"}
    
    L2 -->|"是自己的资源"| PASS[放行]
    L2 -->|"不是自己的资源"| DENY["403/404 拒绝"]
    
    PASS --> L3["第3层: 文件系统安全<br/>✅ 路径遍历防护<br/>✅ 扩展名白名单(33种)<br/>✅ 大小限制(10MB/100MB)<br/>✅ 文件数限制(≤50)"]
    
    L3 --> L4["第4层: 业务逻辑保护<br/>✅ Public Skill 写保护<br/>✅ Reference Skill 只读约束<br/>✅ Skill 名称唯一性与格式校验"]
    
    L4 --> L5["第5层: 可选安全增强<br/>⚙️ 下载 AES-256-GCM 加密<br/>⚙️ API 速率限制<br/>⚙️ 审计日志追踪"]

    style L1 fill:#e8f5e9
    style L2 fill:#e3f2fd
    style L3 fill:#fff3e0
    style L4 fill:#f3e5f5
    style L5 fill:#efebe9
```
...
**第 2 层 —— 所有权隔离**：即使通过了认证，用户也只能操作属于自己的资源。Skill 的上传、更新、删除、停用都会调用 `_ensure_owner()` 检查 `skill.user_id == current_user.id`。Token 和仪表盘数据同样按 user_id 过滤。

**第 3 层 —— 文件系统安全**：防止通过恶意文件路径进行目录穿越攻击（拦截 `../`）、限制可上传的文件类型、控制单个 Skill 的总体积和文件数量，避免滥用存储资源。

**第 4 层 —— 业务逻辑守卫**：公共 Skill（visibility=public）拒绝普通用户的写操作；Reference Skill 拦截所有文件上传和元数据修改请求，返回 409 CONFLICT + `REFERENCE_SKILL_READ_ONLY` 错误码。

**第 5 层 —— 可选加固**：下载加密、速率限制和审计日志都是可选功能，通过各自的 `ENABLE_*` 开关控制。
...
| 场景 | 推荐配置 | 理由 |
|------|----------|------|
| 个人开发机 | 保持默认全部 False | 最简配置，仅自己使用 |
| 小团队内网 | 开启 EMAIL_OTP + AUDIT_LOG | 多人协作需要登录和操作追溯 |
| 对外演示环境 | 开启 PUBLIC_SIGNUP + EMAIL_OTP + VISIBILITY | 让访客能注册并看到预置的公共 Skill |
| 生产部署（建议开 RBAC） | `ENABLE_RBAC=True` + 合理分配角色 | 多用户环境下必须开启细粒度权限 |
...
| 安全防护层数 | **5 层**（JWT / 所有权 / 文件系统 / 业务逻辑 / 可选加固） |
| RBAC 关闭后有实质权限提升的操作 | **6 个**（均为原 admin 专属） |
