...
在 `backend/models/skill.py` 中添加与 SkillVersion 的关系：
...
在 `backend/models/skill_version.py` 中添加 `back_populates`：
...
```bash
git add backend/models/skill.py backend/models/skill_version.py tests/test_skill_service.py
...
**Files:**
- Create: `backend/db/migrations/versions/j5k6l7m8n9o0_add_cascade_delete.py`

**Step 1: 创建迁移脚本**

创建新迁移文件 `backend/db/migrations/versions/j5k6l7m8n9o0_add_cascade_delete.py`：
...
Run: `python -c "from backend.db.migrations.versions.j5k6l7m8n9o0_add_cascade_delete import upgrade, downgrade; print('OK')"`
Expected: OK

**Step 3: Commit**

```bash
git add backend/db/migrations/versions/j5k6l7m8n9o0_add_cascade_delete.py
...
Run: `python -c "from backend.models import User, Skill, SkillVersion, APIToken; print('Models imported successfully')"`
