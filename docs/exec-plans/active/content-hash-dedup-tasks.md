...
- [ ] **T1.1** 新增 Alembic 迁移：`skill_versions` 表加 `content_hash VARCHAR(64) NOT NULL DEFAULT ""`
  - 文件：`backend/db/migrations/versions/`
  - 验证：`uv run alembic -c backend/alembic.ini upgrade head` 成功

- [ ] **T1.2** 更新 `SkillVersion` 模型，新增 `content_hash` 字段
  - 文件：`backend/models/skill_version.py`
  - 验证：`uv run mypy backend` 类型检查通过（或与基线一致）

- [ ] **T1.3** 实现 `compute_skill_content_hash()` 工具函数
...
- [ ] **T1.6** 在 `sync_public_skills.py` 版本创建/补录路径中调用 hash 计算
...
- [ ] **T1.7** 编写回填脚本 `backend/scripts/backfill_content_hash.py`
...
- [ ] **T2.1** `ClientSkillSummaryResponse` 顶层增加 `content_hash` 字段
  - 文件：`backend/schemas/client_skill.py`
  - 类型：`str | None = None`
  - 约束：不要把 `content_hash` 加到共享的 `SkillVersionResponse`

- [ ] **T2.2** `ClientSkillCatalogService` 填充 `content_hash` 值
...
- [ ] **T3.6** 扩展 agent adapter 读取本地 content hash
...
- [ ] **T3.7** 更新 pre-distribution check
...
- [ ] **T3.8** 更新 UI、Electron 主进程和 i18n 文案
...
- [ ] **T3.9** 更新 local skill inventory 受影响类型
...
- [ ] **T4.4** 回填脚本验证
  - 在测试环境运行 `uv run python backend/scripts/backfill_content_hash.py`
