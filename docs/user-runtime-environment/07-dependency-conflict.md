...
def _pick_simulated_version(
    ...
    Note:
        ...
        对于仅含下限约束的 specifier（`>=`, `==`, `~=`, `>`, `===`），上述安全保证成立。
        但当 specifier 包含上限约束（`<`, `<=`）或排除约束（`!=`）时，
        可能产生假阴性，详见下方「约束操作符覆盖范围」子节。
    """
    ...
    if not lower_bounds:
        # 无下限约束（如仅含 !=, <, <= 等操作符），退化处理：
        # - current 存在：返回 current + patch bump
        # - current 为 None：返回 None（调用方跳过该依赖）
    ...
    # 注：若 specifier 包含 <, <=, != 等非下限约束，可能产生假阴性。
...
函数仅从 specifier 中提取下限版本，不处理上限和排除约束。各操作符的处理方式如下：

| 操作符 | 处理方式 | 安全性 |
|--------|---------|--------|
| `>=`, `==`, `~=` | 取版本值作为下限 | 安全方向 |
| `===` | 同上（任意相等，不做版本规范化） | 安全方向 |
| `>` | patch bump 后作为下限（如 `>1.0` → `1.0.1`） | 安全方向 |
| `<`, `<=` | **未处理**（忽略上限） | 见下方说明 |
| `!=` | **未处理**（忽略排除） | 见下方说明 |

**`<` / `<=` 的已知局限**：当 specifier 仅含上限约束时，函数无法收集到下限，会退化到 `current + patch bump`。若当前版本高于上限（降级场景），模拟版本反而高于当前版本，产生假阴性。例如 `requests<=2.28.0`，当前 `2.31.0`，模拟结果为 `2.31.1`，而 pip 实际会降级到 `2.28.0`。此为已知局限性，待后续优化。

**`!=` 的已知局限**：排除约束被静默忽略，模拟版本可能恰好是被排除的版本。例如 `>=2.28.0,!=2.35.0`，模拟结果为 `2.28.0`（满足约束），但若多个下限的最大值恰好是排除版本，则会产生误判。

**`current=None` 且无下限时的语义**：函数返回 `None`，调用方 `detect_upgrade_impact` 会跳过该依赖（`if new_version is not None`），即该包保持 `installed` 中的当前版本不变。
