...
- Completed refresh token hardening: backend refresh tokens are now persisted as
  single-use token-family sessions with reuse detection, user-wide JWT
  revocation on compromise, logout session revocation, and regression tests.
...
| Documentation freshness automation | Planned | Docs structure and indexes are validated, but freshness still relies on manual gardening | `docs/design-docs/documentation-freshness-automation.md`, `docs/exec-plans/completed/documentation-freshness-automation-plan.md` | A script or CI check reports stale plan/index/doc references beyond basic path validation |
