...
async def test_user_execution_slot_blocks_second_acquire_when_limit_one(monkeypatch):
    ...
    from backend.core.utils.execution_control import acquire_execution_slot
...
def test_workdir_quota_rejects_oversized_directory(tmp_path):
    from backend.core.utils.execution_control import is_within_workdir_quota
...
def test_output_truncation_applies_hard_limit():
    from backend.core.utils.execution_control import truncate_output
