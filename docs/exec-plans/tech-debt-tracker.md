...
| Backend mypy baseline failure | `uv run mypy backend` currently fails with existing type errors, led by `enum.StrEnum` under the configured Python 3.13 target and `SkillErrorCode` cascades, so the backend type gate cannot be used as a reliable completion signal yet | `docs/exec-plans/active/client-skills-upload-plan.md` |
