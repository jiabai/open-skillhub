...
2. 聚合行显示：SKILL 名称、路径标签、版本信息、来源代理。
3. 版本冲突检测：同一名称但不同版本的 SKILL 不合并为同一行，以警告形式提示。
4. 上传/删除操作作用于组内全部副本，保持操作语义清晰。
...
1. 用户在全局 `~/.agents/skills/` 和项目级 `project/.agents/skills/` 下各有一份 `review-skill`，Local Skills 视图只显示一行，路径标签展示两个路径。
2. 用户同名但不同版本的 SKILL（如一个 v1.0、一个 v1.2），显示为两行独立记录，版本列标注冲突警告。
3. 用户点击删除时，组内所有路径下的副本都被删除。
4. 用户点击上传时，组内第一个可上传的副本被上传（内容相同，上传其中一个即可）。
...
- [ ] 同名同版本 SKILL 在视图中显示为单行，路径以标签形式展示。
